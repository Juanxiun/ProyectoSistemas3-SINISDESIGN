...
import { ProyecectoList, ProyectosPresentacion } from "../query/list.query.ts";

export class ListProyecto {
  public async List(ctx: Context, usr?: string) {
    const proyectos = await ProyecectoList(usr);
    const proy: ProyectoList[] = proyectos.data as ProyectoList[] ?? [];
  ...
  public async ListView(ctx: Context) {
    const response = await ProyectosPresentacion();
    const proy: ProyectoViewList[] = response.data as ProyectoViewList[] ?? [];
